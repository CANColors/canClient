     ...
     // if (strcmp(commandType, "canpackage"))
      {
        ...
        canM.msg.data_length_code = cJSON_GetObjectItem(command,"dl")->valueint;
          ...
          if (cJSON_IsArray(data) == 1)
          {
             ...
             ESP_LOGI("CAN_SEND", " CAN data Array size %d", data_len); 
             for (int j=0;j<data_len; j++)
             {
                 canM.msg.data[j] =  cJSON_GetArrayItem(data, j)->valueint;
             }
          }
           ESP_LOGI("Sent to CAN", "=> address: %d, dl: %d, data[0]:%d", canM.msg.identifier, canM.msg.data_length_code, canM.msg.data[0]);
