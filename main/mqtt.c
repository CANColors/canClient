...
void mqtt_receive(esp_mqtt_event_handle_t event)
{
  ...
  //  if (!strcmp(event->topic, "request") )
     ...
     if (receptionEnabled == TRUE)
     {
     ESP_LOGI(TAG, "receptionEnabled");
        data =   event->data;
        if (data != NULL)
        {
        ESP_LOGI(TAG, "data present");
...
/*                                                                           */
/*****************************************************************************/ 
 void receptionStateSet(char st)
 {
    if (st == TRUE)  receptionEnabled = TRUE;
    
 
 }
 
 void transmitionStateSet(char st)
 {
  switch (st)
  {
    //case ONCE:
